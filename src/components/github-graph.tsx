...
export function buildContributionWeeks(
  ...
  const endDate = new Date(Math.max(lastDate.getTime(), today.getTime()));
...
function selectRecentContributions(
  ...
  const daysToSubtract = Math.round(Math.max(1, Math.min(12, months)) * 30.4375);
            ...
            {weeks.map((week, weekIndex) => (
              ...
              >
                {week.map((contribution, dayIndex) => {
                  ...
                  const isHovered = hoveredContribution && 
                    weekIndex === hoveredContribution.weekIndex && 
                    dayIndex === hoveredContribution.dayIndex;
                  const filter = isHovered ? `brightness(1.2) saturate(1.1)` : `brightness(1) saturate(1)`;
                  return (
                    <motion.button
            ...
            {typeof document !== "undefined" && createPortal(
              <AnimatePresence>
                {hoveredContribution && (
                  <motion.span
                    role="tooltip"
                    className="pointer-events-none fixed z-[9999] whitespace-nowrap rounded-full bg-foreground px-3 py-1.5 text-sm font-medium text-background ring-1 ring-foreground/15"
                    initial={{
                      opacity: 0,
                      scale: 0.92,
...
export function GithubGraphLegend({
  variant = "github",
  cellSize = 18,
  cellRadius = 3,
  className,
}: Pick<GithubGraphProps, "variant" | "cellSize" | "cellRadius" | "className">) {
  const colors = VARIANTS[variant];
  const resolvedCellRadius = Math.max(
    0,
    Math.min(cellRadius, Math.max(0, cellSize) / 2),
  ...
  return (
    <div
      className={cn("mt-4 flex items-center gap-1.5 text-sm text-muted-foreground", className)}
